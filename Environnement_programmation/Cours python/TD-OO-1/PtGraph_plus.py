...
class PtGraph(PtGeom) :
    ...
    def __init__(self, label, xy=[0,0], marker='o'):
        ...
        if isinstance(label,str):
            ...
            PtGeom.__init__(self,label.getXY())
            self.__label = label.getLabel()
            self.__mrk = label.getMarker()
            self.__size = label.getSize()
            self.__color = label.getColor()
            self.__fillColor = label.getFillColor()
